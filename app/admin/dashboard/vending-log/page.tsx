...
export default function VendingCenterLoggerPage() {
  ...
  // ─── Home Delivery Razorpay Link & WhatsApp Modal State ───
  const [deliveryModalOpen, setDeliveryModalOpen] = useState(false);
  const [deliveryCustName, setDeliveryCustName] = useState("");
  const [deliveryCustPhone, setDeliveryCustPhone] = useState("");
  const [deliveryCustEmail, setDeliveryCustEmail] = useState("");
  const [deliveryProductType, setDeliveryProductType] = useState<"Gutted" | "Non Gutted" | "Live Trout">("Gutted");
  const [deliveryWeight, setDeliveryWeight] = useState("1.000");
  const [deliveryRate, setDeliveryRate] = useState("580");
  const [deliveryAmount, setDeliveryAmount] = useState("580");
  const [deliveryAmountOverridden, setDeliveryAmountOverridden] = useState(false);
  const [deliveryAddressNotes, setDeliveryAddressNotes] = useState("");
  const [deliveryAutoLogSale, setDeliveryAutoLogSale] = useState(true);
  const [deliveryGenerating, setDeliveryGenerating] = useState(false);
  const [deliverySuccessLink, setDeliverySuccessLink] = useState<{
    short_url: string;
    id: string;
    orderRef: string;
    amount: number;
  ...
  const [deliveryCopiedLink, setDeliveryCopiedLink] = useState(false);
  const [deliveryCopiedMsg, setDeliveryCopiedMsg] = useState(false);
  ...
  // ─── Home Delivery Payment Link Handlers ───
  const openHomeDeliveryModal = (entry?: VendingSalesEntry) => {
    setDeliveryError(null);
    setDeliverySuccessLink(null);
    setDeliveryCopiedLink(false);
    setDeliveryCopiedMsg(false);

    if (entry) {
      let extractedName = "";
      let extractedPhone = "";
      if (entry.notes) {
        const phoneMatch = entry.notes.match(/(\+?91[\-\s]?)?[6-9]\d{9}/);
        if (phoneMatch) extractedPhone = phoneMatch[0].replace(/\D/g, "").slice(-10);
        const nameMatch = entry.notes.match(/(?:to|cust|customer|for)[:\s]+([A-Za-z\s]{3,25})/i);
        if (nameMatch) extractedName = nameMatch[1].trim();
      }
      setDeliveryCustName(extractedName);
      setDeliveryCustPhone(extractedPhone);
      setDeliveryCustEmail("");
      const pType = (entry.product_type || "Gutted") as any;
      setDeliveryProductType(pType);
      setDeliveryWeight(entry.weight_kg ? entry.weight_kg.toString() : "1.000");
      setDeliveryRate(entry.rate_per_kg ? entry.rate_per_kg.toString() : "580");
      setDeliveryAmount(entry.amount_paid ? entry.amount_paid.toString() : "580");
      setDeliveryAmountOverridden(true);
      setDeliveryAddressNotes(entry.notes || "");
      setDeliveryAutoLogSale(false);
    } else {
      setDeliveryCustName("");
      setDeliveryCustPhone("");
      setDeliveryCustEmail("");
      setDeliveryProductType("Gutted");
      setDeliveryWeight("1.000");
      setDeliveryRate(guttedPrice ? guttedPrice.toString() : "580");
      setDeliveryAmount(guttedPrice ? guttedPrice.toString() : "580");
      setDeliveryAmountOverridden(false);
      setDeliveryAddressNotes("");
      setDeliveryAutoLogSale(true);
    }
    setDeliveryModalOpen(true);
  };

  const handleDeliveryWeightChange = (newWeight: string) => {
    setDeliveryWeight(newWeight);
    if (!deliveryAmountOverridden) {
      const w = parseFloat(newWeight) || 0;
      const r = parseFloat(deliveryRate) || 0;
      setDeliveryAmount(Math.round(w * r).toString());
    }
  };

  const handleDeliveryRateChange = (newRate: string) => {
    setDeliveryRate(newRate);
    if (!deliveryAmountOverridden) {
      const w = parseFloat(deliveryWeight) || 0;
      const r = parseFloat(newRate) || 0;
      setDeliveryAmount(Math.round(w * r).toString());
    }
  };

  const handleDeliveryProductChange = (newProd: "Gutted" | "Non Gutted" | "Live Trout") => {
    setDeliveryProductType(newProd);
    let rate = "580";
    if (newProd === "Gutted") rate = guttedPrice ? guttedPrice.toString() : "580";
    else if (newProd === "Non Gutted") rate = nonGuttedPrice ? nonGuttedPrice.toString() : "550";
    else rate = nonGuttedPrice ? nonGuttedPrice.toString() : "550";
  ...
  const handleGenerateDeliveryPaymentLink = async () => {
    const amt = parseFloat(deliveryAmount);
    if (!amt || amt <= 0) {
      setDeliveryError("Please specify a valid payable amount (₹ > 0).");
      return;
    }
    const cleanPhone = deliveryCustPhone.replace(/\D/g, "").slice(-10);
    if (!cleanPhone || cleanPhone.length !== 10) {
      setDeliveryError("Please enter a valid 10-digit WhatsApp phone number for the customer.");
      return;
    }

    setDeliveryGenerating(true);
    setDeliveryError(null);

    try {
      const orderRef = `VND-DEL-${Date.now().toString().slice(-6)}`;
      const itemsSummary = `${deliveryWeight} Kg ${deliveryProductType} Trout`;
      ...
      const res = await fetch("/api/razorpay/payment-link", {
        ...
        body: JSON.stringify({
          amount: amt,
          customerName: deliveryCustName.trim() || "Customer",
          customerPhone: cleanPhone,
          customerEmail: deliveryCustEmail.trim() || undefined,
          orderRef,
          itemsSummary,
          notes: deliveryAddressNotes ? `Delivery: ${deliveryAddressNotes}` : "Vending Center Home Delivery",
        }),
      });
      ...
      const waText = `🐟 *URBAN TROUT — FRESH HIMALAYAN RAINBOW TROUT*
━━━━━━━━━━━━━━━━━━━━
Hi ${deliveryCustName.trim() || "there"}! Your fresh trout home delivery order is ready for dispatch:

📦 *Order Ref:* #${orderRef}
🐟 *Product:* ${deliveryProductType} Trout
⚖️ *Net Weight:* ${deliveryWeight} Kg
💰 *Total Payable:* ₹${amt.toLocaleString("en-IN")} (Locked Amount)
${deliveryAddressNotes ? `📍 *Delivery Note:* ${deliveryAddressNotes}\n` : ""}
🔒 *Click here to pay securely via UPI, Google Pay, PhonePe, Paytm, or Card:*
👉 ${short_url}

_✅ No manual screenshots required. Your payment is verified instantly, auto-confirming your delivery._
📞 Srinagar Support: +91 84910 06127`;

      setDeliverySuccessLink({
        short_url,
        id,
        orderRef,
        amount: amt,
      ...
      if (deliveryAutoLogSale) {
        try {
          const payload = {
            entry_date: getTodayDate(),
            entry_time: getCurrentTime(),
            weight_kg: parseFloat(deliveryWeight) || 1.0,
            product_type: deliveryProductType,
            rate_per_kg: parseFloat(deliveryRate) || 580,
            expected_amount: amt,
            amount_paid: 0,
            discount_amount: 0,
            payment_mode: "Razorpay Link",
            logged_by: "Delivery Dispatch",
            notes: `[HOME DELIVERY #${orderRef}] ${deliveryCustName.trim()} (${cleanPhone}) - Link: ${short_url} ${deliveryAddressNotes ? `| Note: ${deliveryAddressNotes}` : ""}`.trim(),
            custom_fields: {
              payment_status: "PENDING_LINK",
              order_ref: orderRef,
              payment_link_id: id,
              payment_link_url: short_url,
              customer_name: deliveryCustName.trim(),
              customer_phone: cleanPhone,
              is_home_delivery: true,
            },
          };

          const logRes = await fetch("/api/vending-log", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
          });
          ...
          if (logData.entry) {
            setEntries((prev) => [logData.entry, ...prev]);
          }
        } catch (logErr) {
          console.warn("Could not auto-log pending dispatch:", logErr);
        }
      }

      playLogChime();
    } catch (err: any) {
      setDeliveryError(err.message || "Failed to generate payment link.");
    } finally {
      setDeliveryGenerating(false);
    }
  };
    ...
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6 pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            type="button"
            onClick={() => openHomeDeliveryModal()}
            className="py-2.5 px-3.5 rounded-xl bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white text-xs font-bold transition-all shadow-lg shadow-cyan-600/25 flex items-center gap-1.5 cursor-pointer active:scale-95"
            title="Generate locked Razorpay Payment Link for Home Delivery WhatsApp sharing"
          >
            <span className="material-symbols-outlined text-base">delivery_dining</span>
            <span>Home Delivery Link</span>
          </button>
                    ...
                    >
                      ...
                      <td className="py-3 px-3 text-center whitespace-nowrap">
                        <div className="flex items-center justify-center gap-1 opacity-80 group-hover:opacity-100">
                          <button
                            type="button"
                            onClick={() => openHomeDeliveryModal(e)}
                            className="p-1 rounded-lg bg-slate-800 hover:bg-cyan-950/70 text-cyan-400 hover:text-cyan-300 transition-all cursor-pointer"
                            title="Generate Home Delivery Razorpay Link & WhatsApp Bill"
                          >
      ...
      {/* ══════════════════════════════════════════════════════════
          MODAL 7: HOME DELIVERY RAZORPAY LINK & WHATSAPP BILL
          ══════════════════════════════════════════════════════════ */}
      {deliveryModalOpen && (
        <div
          onClick={(e) => {
            if (e.target === e.currentTarget) setDeliveryModalOpen(false);
          }}
          className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-md overflow-y-auto"
        >
          <div className="bg-slate-900 border border-cyan-500/40 rounded-3xl p-5 sm:p-7 max-w-2xl w-full text-slate-200 shadow-2xl space-y-5 my-4 max-h-[92vh] overflow-y-auto">
            {/* Modal Header */}
            <div className="flex items-start justify-between border-b border-slate-800 pb-3.5">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-2xl bg-cyan-500/20 border border-cyan-500/30 flex items-center justify-center text-cyan-400 flex-shrink-0">
                  <span className="material-symbols-outlined text-2xl">delivery_dining</span>
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-base sm:text-lg font-black text-white font-['Space_Grotesk']">
                      Home Delivery Payment Link
                    </h3>
                    <span className="px-2 py-0.5 rounded-full bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 text-[10px] font-bold font-mono">
                      🔒 Anti-Fraud
                    </span>
                  </div>
                  <p className="text-xs text-slate-400 mt-0.5">
                    Generate locked Razorpay link &amp; WhatsApp bill. Auto-alerts Telegram &amp; Email upon payment.
                  </p>
                </div>
              </div>
              <button
            ...
            {/* Error Message */}
            {deliveryError && (
              <div className="p-3 rounded-xl bg-rose-500/20 border border-rose-500/40 text-rose-200 text-xs font-mono flex items-center gap-2">
                <span className="material-symbols-outlined text-base text-rose-400">error</span>
                <span>{deliveryError}</span>
              </div>
            )}

            {/* Success State: Generated Link & 1-Click WhatsApp Button */}
            {deliverySuccessLink ? (
              <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
                <div className="p-4 rounded-2xl bg-emerald-950/40 border border-emerald-500/40 space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse"></span>
                      <span className="text-xs uppercase font-black tracking-wider text-emerald-300 font-mono">
                        Payment Link Ready!
                      </span>
                    </div>
                    <span className="px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 text-[10px] font-bold font-mono">
                      Ref #{deliverySuccessLink.orderRef}
                    </span>
                  </div>

                  <div className="bg-slate-950/80 rounded-xl p-3 border border-emerald-500/30 flex items-center justify-between gap-3 font-mono">
                    <div className="truncate">
                      <div className="text-[10px] text-slate-400 uppercase tracking-wider">Locked Single-Use Link</div>
                      <div className="text-emerald-400 font-bold text-sm truncate">{deliverySuccessLink.short_url}</div>
                    </div>
                    <button
                      type="button"
                      onClick={() => {
                        navigator.clipboard.writeText(deliverySuccessLink.short_url);
                        setDeliveryCopiedLink(true);
                        setTimeout(() => setDeliveryCopiedLink(false), 2500);
                      }}
                      className="px-2.5 py-1.5 rounded-lg bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-200 text-xs font-bold transition-all flex items-center gap-1 cursor-pointer flex-shrink-0"
                    >
                      <span className="material-symbols-outlined text-xs">
                        {deliveryCopiedLink ? "check" : "content_copy"}
                      </span>
                      <span>{deliveryCopiedLink ? "Copied!" : "Copy Link"}</span>
                    </button>
                  </div>

                  {/* Primary WhatsApp Action Button */}
                  <a
                    href={`https://wa.me/91${deliverySuccessLink.customerPhone}?text=${encodeURIComponent(deliverySuccessLink.waMessage)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-slate-950 text-xs font-black uppercase tracking-wider transition-all flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/25 active:scale-95 cursor-pointer"
                  >
                    <span className="material-symbols-outlined text-base">chat</span>
                    <span>📲 Send via WhatsApp to +91 {deliverySuccessLink.customerPhone}</span>
                  </a>

                  {/* Message Preview */}
                  <div className="space-y-1">
                    <div className="flex items-center justify-between text-[10px] font-mono text-slate-400">
                      <span>WhatsApp Bill Preview:</span>
                      <button
                        type="button"
                        onClick={() => {
                          navigator.clipboard.writeText(deliverySuccessLink.waMessage);
                          setDeliveryCopiedMsg(true);
                          setTimeout(() => setDeliveryCopiedMsg(false), 2500);
                        }}
                        className="text-cyan-400 hover:text-cyan-300 underline cursor-pointer"
                      >
                        {deliveryCopiedMsg ? "✓ Copied!" : "Copy Message Text"}
                      </button>
                    </div>
                    <div className="p-3 bg-slate-950/70 border border-slate-800 rounded-xl text-[11px] font-mono text-slate-300 whitespace-pre-wrap leading-relaxed max-h-40 overflow-y-auto">
                      {deliverySuccessLink.waMessage}
                    </div>
                  </div>

                  {/* Auto-Verification Banner */}
                  <div className="p-3 rounded-xl bg-slate-900 border border-slate-800 text-[11px] text-slate-300 font-mono flex items-start gap-2">
                    <span className="material-symbols-outlined text-base text-cyan-400 flex-shrink-0">verified</span>
                    <span>
                      <strong>Automated Verification:</strong> Once {deliverySuccessLink.customerName || "the customer"} completes payment on UPI/Card, our webhook will immediately trigger a <strong>Telegram alert</strong> with a 1-tap customer WhatsApp reply button, send a receipt email to <strong>info.urbantrout@gmail.com</strong>, and automatically mark the dispatch as <strong>PAID ✓</strong>.
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-2">
                  <button
                    ...
                    className="py-2 px-3.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold transition-all cursor-pointer"
                  >
                    ← Create Another Link
                  </button>
                  <button
                    type="button"
                    onClick={() => setDeliveryModalOpen(false)}
                    className="py-2 px-4 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-bold transition-all cursor-pointer"
                  >
                    Done &amp; Close
                  </button>
                </div>
              ...
              <div className="space-y-4 font-mono">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      type="text"
                      value={deliveryCustName}
                      onChange={(e) => setDeliveryCustName(e.target.value)}
                      placeholder="e.g. Tariq Ahmad"
                      className="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-400"
                    />
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-1.5 bg-slate-950 border border-slate-700 rounded-xl px-2.5 py-1.5 focus-within:border-cyan-400">
                      <span className="text-xs text-slate-500 font-bold">+91</span>
                      <input
                        type="tel"
                        value={deliveryCustPhone}
                        onChange={(e) => setDeliveryCustPhone(e.target.value.replace(/\D/g, "").slice(0, 10))}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <select
                      value={deliveryProductType}
                      onChange={(e) => handleDeliveryProductChange(e.target.value as any)}
                      className="w-full px-2.5 py-2 bg-slate-950 border border-slate-700 rounded-xl text-xs text-white focus:outline-none focus:border-cyan-400"
                    >
                      <option value="Gutted">Gutted Trout (Cleaned)</option>
                      <option value="Non Gutted">Non Gutted Trout</option>
                      <option value="Live Trout">Live Trout</option>
                    </select>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      onChange={(e) => handleDeliveryWeightChange(e.target.value)}
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      step="1"
                      value={deliveryRate}
                      onChange={(e) => handleDeliveryRateChange(e.target.value)}
                      className="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-xs text-white focus:outline-none focus:border-cyan-400"
                    />
                  </div>
                </div>

                {/* Total Payable & Customer Email */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-7 pr-3 py-2 bg-slate-950 border border-cyan-500/50 rounded-xl text-sm font-bold text-emerald-400 focus:outline-none focus:border-cyan-400"
                      />
                    </div>
                    <span className="text-[10px] text-slate-500 block">
                      Amount locked in Razorpay link; customer cannot alter it.
                    </span>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      type="email"
                      value={deliveryCustEmail}
                      onChange={(e) => setDeliveryCustEmail(e.target.value)}
                      placeholder="customer@gmail.com"
                      className="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-400"
                    />
                    <span className="text-[10px] text-slate-500 block">
                      Receives digital HTML receipt automatically upon payment.
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) => setDeliveryAddressNotes(e.target.value)}
                    placeholder="e.g. House #14, Rajbagh near Abdullah Bridge"
                    className="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-400"
                  />
                </div>

                {/* Auto Log Sale Checkbox */}
                <label className="flex items-center gap-2 p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-xs text-slate-300 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={deliveryAutoLogSale}
                    onChange={(e) => setDeliveryAutoLogSale(e.target.checked)}
                    className="rounded border-slate-700 text-cyan-500 focus:ring-cyan-500 w-4 h-4 cursor-pointer"
                  />
                  <span>
                    Auto-record this dispatch in Vending Center sales ledger (marked as <strong>⏳ Link Sent</strong> until paid)
                  </span>
                </label>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-slate-800">
                  ...
                  <button
                    type="button"
                    disabled={deliveryGenerating}
                    onClick={handleGenerateDeliveryPaymentLink}
                    className="py-2.5 px-5 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white text-xs font-black uppercase tracking-wider transition-all disabled:opacity-60 cursor-pointer flex items-center gap-2 shadow-lg shadow-cyan-500/25 active:scale-95"
                  >
                    {deliveryGenerating ? (
                      <span className="w-4 h-4 border-2 border-white border-t-transparent animate-spin rounded-full" />
                    ) : (
                    ...
                    <span>{deliveryGenerating ? "Creating Razorpay Link…" : "Generate Locked Link & WhatsApp"}</span>
                  </button>
                </div>
              </div>
            )}
