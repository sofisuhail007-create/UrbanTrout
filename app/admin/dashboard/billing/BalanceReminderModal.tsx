...
export default function BalanceReminderModal({
  ...
  const [partialPaidSuccess, setPartialPaidSuccess] = useState<{ remainingBalance: number } | null>(null);
  ...
  const handleRecordRepayment = async () => {
    ...
    try {
      ...
      if (data?.success) {
        ...
        // If partial payment with remaining balance, stay open and show quick next-step actions
        const newRemainingBalance = Math.max(0, record.balance_amount - amount);
        if (newRemainingBalance > 0) {
          setPartialPaidSuccess({ remainingBalance: newRemainingBalance });
          setPaymentAmount("");
          setSettlementNote("");
        } else {
          onClose();
        ...
        <div className="p-4 overflow-y-auto flex-1 space-y-4">
          ...
          {activeTab === "settle" && (
            <div className="space-y-4">
              {/* POST PARTIAL PAYMENT QUICK ACTIONS BANNER */}
              {partialPaidSuccess && (
                <div className="p-3.5 bg-gradient-to-br from-emerald-950/60 via-slate-900 to-slate-900 border border-emerald-500/50 rounded-2xl space-y-3 animate-fadeIn shadow-lg">
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-xl bg-emerald-500/20 border border-emerald-500/40 text-emerald-400 flex items-center justify-center font-bold text-lg">
                      ✓
                    </div>
                    <div>
                      <p className="text-xs font-bold text-emerald-300">Cash Payment Recorded!</p>
                      <p className="text-[11px] text-slate-400 font-mono">
                        Remaining balance: <span className="text-amber-400 font-bold">₹{partialPaidSuccess.remainingBalance.toLocaleString("en-IN")}</span>
                      </p>
                    </div>
                  </div>
                  <p className="text-[11px] text-slate-300 font-mono">
                    How would you like to collect the remaining ₹{partialPaidSuccess.remainingBalance.toLocaleString("en-IN")}?
                  </p>
                  <div className="grid grid-cols-1 gap-2">
                    {/* Option A: Send Razorpay Link on WhatsApp */}
                    <button
                      type="button"
                      onClick={() => {
                        setPartialPaidSuccess(null);
                      ...
                      className="w-full py-2.5 px-3 rounded-xl bg-gradient-to-r from-green-500/20 to-emerald-500/20 hover:from-green-500/30 hover:to-emerald-500/30 border border-emerald-500/50 text-emerald-300 text-xs font-bold flex items-center justify-center gap-2 transition-all cursor-pointer"
                    >
                      <span className="text-base">📲</span>
                      Send Razorpay Payment Link on WhatsApp (₹{partialPaidSuccess.remainingBalance.toLocaleString("en-IN")})
                    </button>
                    {/* Option B: Done - close */}
                    <button
                      type="button"
                      onClick={onClose}
                      className="w-full py-2 px-3 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 text-xs font-bold flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                    >
                      <span className="material-symbols-outlined text-sm">check_circle</span>
                      Done — Close (Collect remaining later)
                    </button>
                  </div>
                </div>
              )}
              ...
              {!partialPaidSuccess && (
                <div className="p-3.5 bg-slate-950 border border-slate-800 rounded-xl space-y-3">
                  ...
                  {/* Show remaining balance preview if partial */}
                  {paymentAmount && parseFloat(paymentAmount) > 0 && parseFloat(paymentAmount) < record.balance_amount && (
                    <div className="p-2 rounded-lg bg-amber-950/30 border border-amber-500/30 text-[11px] text-amber-300 font-mono">
                      Remaining after this payment: ₹{(record.balance_amount - parseFloat(paymentAmount)).toLocaleString("en-IN")} — you'll get quick options to collect it via Razorpay/WhatsApp.
                    </div>
                  )}
